...
export default function LeaderboardScreen() {
  ...
  const contentMargin = isMobile ? 16 : 150;
  ...
  const fetchLeaderboard = useCallback(async (pageNum: number, searchQuery: string, showLoading = false) => {
    if (showLoading) {
      setLoading(true);
    }
    // Don't clear error/data immediately to prevent flash if silent
    try {
      ...
      // Only set error if we don't have data (or if it's user facing)
      // Otherwise keep showing old data silently
      if (showLoading) {
  ...
  // Debounce logic: Update debouncedQuery 500ms after query changes
  useEffect(() => {
  ...
  // Fetch when page or debouncedQuery changes
  useEffect(() => {
    // Show loader ONLY on very first load to avoid annoyance
    const shouldLoad = isFirstLoad.current;

    fetchLeaderboard(page, debouncedQuery, shouldLoad);

    if (isFirstLoad.current) {
      isFirstLoad.current = false;
    }

    // Interval updates are ALWAYS silent (false)
    const interval = setInterval(() => fetchLeaderboard(page, debouncedQuery, false), 6000);
    return () => clearInterval(interval);
  }, [page, debouncedQuery, fetchLeaderboard]);
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Search Bar - Moved */}
        <View style={[styles.searchContainer, { marginHorizontal: contentMargin }]}>
          <TextInput
        ...
        {/* List Header */}
        <View style={[styles.tableHeader, { marginHorizontal: contentMargin }]}>
